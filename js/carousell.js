...
function clickCardContent(cardSelector, location) {
    $(document).on("click", cardSelector, function () {
        ...
        if (data) {
            window.location.href = `${location}.html?find=${encodeURIComponent(data)}`;
        } else {
...
function getDetailCultureTopAtractionData(containerInput){
    ...
    return wisataBudaya.filter(wisata => wisata.destination.toLowerCase() === searchQueryItem.toLowerCase() && wisata.status === "atraksi").forEach((wisata) => {
        let card = `
        <div class="card" data-attribute="${wisata.title}" onclick="redirectToDetail('${wisata.title}', '${wisata.status}')">
            <img src="${wisata.image}" alt="${wisata.title}">
            <h3 class="medium-text-font bold">${wisata.title}</h3>
...
function getDetailCultureTopFestivalData(containerInput){
    ...
    return wisataBudaya.filter(wisata => wisata.destination.toLowerCase() === searchQueryItem.toLowerCase() && wisata.status === "festival").forEach((wisata) => {
        let card = `
        <div class="card" data-attribute="${wisata.title}" onclick="redirectToDetail('${wisata.title}', '${wisata.status}')">
            <img src="${wisata.image}" alt="${wisata.title}">
            <h3 class="medium-text-font bold">${wisata.title}</h3>
